...
export async function sendContact(_prev: ContactFormState, formData: FormData): Promise<ContactFormState> {
  ...
  if (!nombre || !email || !mensaje) {
    return { success: false, error: "Campos requeridos incompletos." };
  }

  try {
    await resend.emails.send({
      from: "Build Haus Studio <noreply@buildhaus.studio>",
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#071A33">
          ...
          <table style="width:100%;border-collapse:collapse">
            ...
            ${web ? `<tr><td style="padding:8px 0;font-weight:600">Web actual</td><td><a href="${web}">${web}</a></td></tr>` : ""}
    ...
    return { success: false, error: "No pudimos enviar tu mensaje. Intentalo de nuevo o escribinos directo." };
  }
}
